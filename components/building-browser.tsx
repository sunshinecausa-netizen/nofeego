  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="mt-2 grid grid-cols-2 items-stretch gap-1.5 border-t border-border/60 pt-2 md:grid-cols-4">
        {[['In-Unit W/D Available', 'In-Unit W/D'], ['Pets Allowed', 'Pets'], ['Gym', 'Gym'], ['Outdoor Space', 'Private Outdoor Space'], ['Elevator', 'Elevator'], ['Doorman', 'Doorman'], ['Pool', 'Swimming Pool']].map(([value, label]) => <button key={value} type="button" aria-pressed={amenities.includes(value)} onClick={() => toggleAmenity(value, !amenities.includes(value))} className={`min-h-10 rounded-md border px-2 text-xs font-semibold leading-tight ${amenities.includes(value) ? 'border-primary bg-primary text-primary-foreground' : 'border-input bg-background hover:border-primary/50'}`}>{label}</button>)}
        <details onMouseLeave={(event) => { if (window.innerWidth >= 768) event.currentTarget.removeAttribute('open'); }} className="group relative">
          <summary className="flex min-h-10 cursor-pointer list-none items-center justify-center gap-1.5 rounded-md border border-input bg-background px-2 text-center text-[11px] font-bold uppercase leading-tight tracking-tight text-primary"><SlidersHorizontal className="h-3.5 w-3.5 shrink-0" /><span>More Filters</span>{activeFilterCount > 0 && <Badge variant="secondary" className="px-1">{activeFilterCount}</Badge>}<ChevronDown className="h-3.5 w-3.5 shrink-0 transition group-open:rotate-180" /></summary>
        ...
        <Button type="submit" className="col-span-2 min-h-10 px-2 text-xs md:col-span-4"><Search className="mr-1.5 h-4 w-4" />Apply</Button>
      </div>
