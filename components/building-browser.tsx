...
export function BuildingBrowser({ initialPage, initialQuery = '', initialFilters, initialResult: result, initialError = null, mode }: Props) {
  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="grid grid-cols-2 items-end gap-2 md:grid-cols-[minmax(0,2fr)_minmax(0,1.6fr)_minmax(130px,auto)_minmax(120px,auto)]">
        <div className="min-w-0"><label htmlFor="building-search" className="mb-1 block text-xs font-medium text-muted-foreground">Search</label><Input id="building-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Building, address, neighborhood" /></div>
