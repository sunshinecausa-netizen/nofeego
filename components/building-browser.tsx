  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="mt-2 grid grid-cols-2 items-stretch gap-1.5 border-t border-border/60 pt-2 md:grid-cols-4">
        {[['In-Unit W/D Available', 'In-Unit W/D'], ['Dishwasher', 'Dishwasher'], ['Gym', 'Gym'], ['Pets Allowed', 'Pets Allowed'], ['Outdoor Space', 'Private Outdoor Space'], ['Pool', 'Swimming Pool'], ['Elevator', 'Elevator'], ['Doorman', 'Doorman']].map(([value, label]) => <button key={value} type="button" aria-pressed={amenities.includes(value)} onClick={() => toggleAmenity(value, !amenities.includes(value))} className={`h-10 rounded-md border px-3 text-sm font-normal leading-tight ${amenities.includes(value) ? 'border-primary bg-primary text-primary-foreground' : 'border-input bg-background hover:border-primary/50'}`}>{label}</button>)}
        <details onMouseLeave={(event) => { if (window.innerWidth >= 768) event.currentTarget.removeAttribute('open'); }} className="group relative md:col-start-3">
          <summary className="flex h-10 cursor-pointer list-none items-center justify-center gap-1.5 rounded-md border border-input bg-background px-3 text-center text-sm font-normal leading-tight text-foreground"><SlidersHorizontal className="h-4 w-4 shrink-0 text-primary" /><span>More Filters</span>{activeFilterCount > 0 && <Badge variant="secondary" className="px-1">{activeFilterCount}</Badge>}<ChevronDown className="h-4 w-4 shrink-0 transition group-open:rotate-180" /></summary>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background md:h-[calc(100dvh-4rem)] md:min-h-0 md:overflow-hidden">
      ...
      <div className="min-h-0 flex-1 md:grid md:grid-cols-2 min-[1100px]:grid-cols-[55fr_45fr]">
        ...
        <section className={`${mobileView === 'list' ? 'flex' : 'hidden'} min-h-0 flex-col bg-muted/25 md:flex`} aria-label="Building results list">
