...
export function BuildingBrowser({ initialPage, initialQuery = '', initialFilters, initialResult: result, initialError = null, mode }: Props) {
  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="mt-2 grid grid-cols-2 items-stretch gap-1.5 border-t border-border/60 pt-2 md:grid-cols-9">
        {[['In-Unit W/D Available', 'In-Unit W/D'], ['Pets Allowed', 'Pets'], ['Gym', 'Gym'], ['Outdoor Space', 'Private Outdoor Space'], ['Elevator', 'Elevator'], ['Doorman', 'Doorman'], ['Pool', 'Swimming Pool']].map(([value, label]) => <button key={value} type="button" aria-pressed={amenities.includes(value)} onClick={() => toggleAmenity(value, !amenities.includes(value))} className={`min-h-10 rounded-md border px-1 text-[10px] font-semibold leading-tight xl:px-1.5 xl:text-[11px] ${amenities.includes(value) ? 'border-primary bg-primary text-primary-foreground' : 'border-input bg-background hover:border-primary/50'}`}>{label}</button>)}
        <details onMouseLeave={(event) => { if (window.innerWidth >= 768) event.currentTarget.removeAttribute('open'); }} className="group relative">
          <summary className="flex min-h-10 cursor-pointer list-none items-center justify-center gap-1 rounded-md border border-input bg-background px-1 text-center text-[9px] font-bold uppercase leading-tight tracking-tight text-primary xl:text-[10px]"><SlidersHorizontal className="hidden h-3.5 w-3.5 shrink-0 xl:block" /><span>More Filters</span>{activeFilterCount > 0 && <Badge variant="secondary" className="hidden px-1 xl:inline-flex">{activeFilterCount}</Badge>}<ChevronDown className="h-3.5 w-3.5 shrink-0 transition group-open:rotate-180" /></summary>
        ...
        <Button type="submit" className="min-h-10 px-1 text-[11px] xl:text-xs"><Search className="mr-1 h-3.5 w-3.5" />Apply</Button>
      </div>
