    ...
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="grid gap-2">
        ...
        <div className="flex min-w-0 flex-nowrap items-end gap-2">
          ...
          <details onMouseLeave={(event) => { if (window.innerWidth >= 768) event.currentTarget.removeAttribute('open'); }} className="group relative w-auto shrink-0">
            <summary className="flex h-10 min-w-max cursor-pointer list-none items-center justify-between gap-2 rounded-md border border-input bg-background px-3 text-sm"><span className="whitespace-nowrap" title={locale === 'zh-Hans' ? '行政区与社区' : 'Borough & Neighborhood'}>{locale === 'zh-Hans' ? '行政区与社区' : 'Borough & Neighborhood'}{boroughs.length + neighborhoods.length > 0 ? ` (${boroughs.length + neighborhoods.length})` : ''}</span><ChevronDown className="h-4 w-4 shrink-0 transition group-open:rotate-180" /></summary>
        ...
        <Button type="submit" className="h-10 min-w-[112px] flex-1"><Search className="mr-2 h-5 w-5" />Search</Button>
        </div>
        </div>
      </div>
