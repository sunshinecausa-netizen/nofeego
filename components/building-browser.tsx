  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <div className="grid min-w-0 flex-[4_1_620px] grid-cols-2 gap-2 lg:grid-cols-5">
          ...
          <details onMouseLeave={(event) => { if (window.innerWidth >= 768) event.currentTarget.removeAttribute('open'); }} className="group relative min-w-0">
            <summary className="flex h-10 cursor-pointer list-none items-center justify-between rounded-md border border-input bg-background px-3 text-sm"><span className="truncate">{locale === 'zh-Hans' ? '社区' : 'Neighborhood'}{boroughs.length + neighborhoods.length > 0 ? ` (${boroughs.length + neighborhoods.length})` : ''}</span><ChevronDown className="h-4 w-4 shrink-0 transition group-open:rotate-180" /></summary>
