...
export function BuildingBrowser({ initialPage, initialQuery = '', initialFilters, initialResult: result, initialError = null, mode }: Props) {
  ...
  const compactFilters = (
    <form onSubmit={onSubmit} role="search" className="border-y border-border bg-white px-3 py-3 shadow-sm sm:px-5">
      ...
      <div className="mt-2 grid grid-cols-2 items-center gap-2 border-t border-border/60 pt-2 xl:grid-cols-4">
        {[['In-Unit W/D Available', 'In-Unit W/D'], ['Pets Allowed', 'Pets'], ['Gym', 'Gym'], ['Outdoor Space', 'Private Outdoor Space'], ['Elevator', 'Elevator'], ['Doorman', 'Doorman'], ['Pool', 'Swimming Pool']].map(([value, label]) => <button key={value} type="button" aria-pressed={amenities.includes(value)} onClick={() => toggleAmenity(value, !amenities.includes(value))} className={`min-h-10 rounded-md border px-3 text-sm font-medium ${amenities.includes(value) ? 'border-primary bg-primary text-primary-foreground' : 'border-input bg-background hover:border-primary/50'}`}>{label}</button>)}
