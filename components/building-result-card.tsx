...
export function BuildingCard({ building, inventory, compared = false, favorited = false, highlighted = false, variant = 'list', onCompareChange, onFavoriteChange, onHover, onSelect, onClose }: Props) {
  ...
  return (
    <article data-building-id={building.id} className={cn('group relative overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-200 focus-within:ring-2 focus-within:ring-primary/40 hover:border-primary/30 hover:shadow-md', compact ? 'max-h-[520px] w-[340px] overflow-y-auto' : 'min-h-[224px] cursor-pointer hover:-translate-y-0.5', highlighted ? 'border-primary ring-2 ring-primary/20' : 'border-border')} onMouseEnter={() => onHover?.(building.id)} onMouseLeave={() => onHover?.(null)} onClickCapture={(event) => { const target = event.target as HTMLElement; if (target.closest('button, a, input, label, [role="checkbox"]')) return; onSelect?.(building.id); }}>
      <button type="button" className={cn('absolute top-3 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-white text-foreground shadow', onClose ? 'right-16' : 'right-3')} aria-label={savedAndCompared ? `Remove ${building.name} from saved and compare` : `Save ${building.name} and add to compare`} title="Save and Compare" aria-pressed={savedAndCompared} onClick={(event) => { event.stopPropagation(); const next = !savedAndCompared; onFavoriteChange?.(building, next); onCompareChange?.(building, next); }}><Heart className={cn('h-5 w-5', (favorited || compared) && 'fill-destructive text-destructive')} /></button>
      ...
      <div className={cn('grid h-full', compact ? 'grid-cols-1' : 'min-h-[224px] grid-cols-1 sm:grid-cols-[41%_59%]')}>
        ...
        <div className="flex min-w-0 flex-col p-3">
          <p className={cn('truncate text-sm font-semibold uppercase tracking-[0.1em] text-primary', onClose ? 'pr-28' : 'pr-14')}>{building.neighborhood ?? building.borough ?? 'New York metro'}</p>
          <h2 className={cn('truncate font-serif text-xl font-bold text-foreground transition group-hover:text-primary', onClose ? 'pr-28' : 'pr-14')}>{building.name}</h2>
          <p className={cn('mb-2 flex items-start gap-1.5 text-sm leading-5 text-muted-foreground', onClose ? 'pr-28' : 'pr-14')}><MapPin className="mt-0.5 h-4 w-4 shrink-0" /><span className="line-clamp-2">{fullAddress}</span></p>
          ...
          <div className="relative z-10 mt-auto border-t border-border/70 pt-2"><p className="mb-2 min-h-8 px-1 text-sm font-semibold text-foreground">Join others interested in sharing this home</p><div className="grid grid-cols-[46fr_54fr] gap-1.5"><Button asChild size="sm" variant="outline" className="h-11 whitespace-nowrap px-2 text-sm leading-none"><Link href={`/roommate-request?${requestContext}`} onClick={(event) => event.stopPropagation()}><Users className="mr-1 h-4 w-4 shrink-0" />Find a Roommate</Link></Button><Button asChild size="sm" className="h-11 whitespace-nowrap px-2 text-sm leading-none"><Link href={`/rent-request?${requestContext}`} onClick={(event) => event.stopPropagation()}><Home className="mr-1 h-4 w-4 shrink-0" />Rent the Entire Unit</Link></Button></div></div>
        </div>
      </div>
    </article>
  );
