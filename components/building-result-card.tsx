    ...
    <article data-building-id={building.id} className={cn('group relative overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-200 focus-within:ring-2 focus-within:ring-primary/40 hover:border-primary/30 hover:shadow-md', compact ? 'max-h-[520px] w-[340px] overflow-y-auto' : 'min-h-[224px] cursor-pointer hover:-translate-y-0.5', highlighted ? 'border-primary ring-2 ring-primary/20' : 'border-border')} onMouseEnter={() => onHover?.(building.id)} onMouseLeave={() => onHover?.(null)} onClickCapture={(event) => { const target = event.target as HTMLElement; if (target.closest('button, a, input, label, [role="checkbox"]')) return; onSelect?.(building.id); }}>
      <div className={cn('grid h-full', compact ? 'grid-cols-1' : 'min-h-[224px] grid-cols-1 sm:grid-cols-[41%_59%]')}>
        ...
        <div className="flex min-w-0 flex-col p-3">
          ...
          <div className="mb-1.5 grid grid-cols-2 gap-1">{CORE_AMENITIES.map((amenity) => { const confirmed = amenity.values.some((value) => amenities.has(value)); return confirmed ? <span key={amenity.label} className="inline-flex items-center gap-1 rounded-md bg-accent px-2 py-1 text-[10px] font-medium text-accent-foreground"><Check className="h-3 w-3 text-primary-hover" />{amenity.label}</span> : <span key={amenity.label} aria-hidden="true" />; })}</div>
          {(building.amenities?.length ?? 0) > 0 && <div className="relative z-10 mb-1"><button type="button" className="flex min-h-7 w-full items-center justify-center gap-1 rounded-md border border-border/70 bg-white px-2 text-[11px] font-semibold text-muted-foreground transition hover:border-primary/40 hover:text-foreground" aria-expanded={showAllAmenities} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setShowAllAmenities((value) => !value); }}>More <ChevronDown className={cn('h-3.5 w-3.5 transition-transform', showAllAmenities && 'rotate-180')} /></button>{showAllAmenities && <div className="mt-1.5 flex flex-wrap gap-1 rounded-md border border-border/60 bg-muted/20 p-2">{building.amenities?.map((amenity) => <span key={amenity} className="rounded-full bg-white px-2 py-1 text-[10px] font-medium text-foreground shadow-sm">{amenity}</span>)}</div>}</div>}

          {subway && <div className="mb-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted-foreground"><span className="inline-flex items-center gap-1"><TrainFront className="h-3 w-3" />{subway}</span></div>}
          <div className="relative z-10 mt-auto border-t border-border/70 pt-2"><label className="mb-2 flex min-h-11 cursor-pointer items-center gap-2 rounded-md px-1 text-xs font-medium text-muted-foreground hover:text-foreground" onClick={(event) => event.stopPropagation()}><Checkbox checked={compared} onCheckedChange={(checked) => onCompareChange?.(building, checked === true)} aria-label={`Save ${building.name} and add it to compare`} />Save and Compare</label><div className="grid grid-cols-2 gap-1.5"><Button asChild size="sm" variant="outline" className="h-10 whitespace-nowrap px-2 text-[10px] leading-none"><Link href={`/roommate-request?${requestContext}`} onClick={(event) => event.stopPropagation()}><Users className="mr-1 h-3.5 w-3.5 shrink-0" />Find a Roommate</Link></Button><Button asChild size="sm" className="h-10 whitespace-nowrap px-2 text-[10px] leading-none"><Link href={`/rent-request?${requestContext}`} onClick={(event) => event.stopPropagation()}><Home className="mr-1 h-3.5 w-3.5 shrink-0" />Rent the Entire Unit</Link></Button></div></div>
        </div>
      </div>
    </article>
  );
