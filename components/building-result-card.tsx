...
export function BuildingCard({ building, inventory, compared = false, favorited = false, highlighted = false, variant = 'list', onCompareChange, onFavoriteChange, onHover, onSelect, onClose }: Props) {
  ...
  const selectedPlan = availablePlans.reduce<(typeof availablePlans)[number] | undefined>((lowest, plan) => !lowest || plan.price < lowest.price ? plan : lowest, undefined);
  const currentOptionsContext = new URLSearchParams({
    buildingId: building.id,
    buildingSlug: building.slug,
    buildingName: building.name,
    neighborhood: building.neighborhood ?? building.borough ?? '',
    address: fullAddress,
    floorPlan: selectedPlan?.label ?? '',
    preferredUnitType: selectedPlan?.label ?? '',
  ...
  return (
    <article data-building-id={building.id} data-card-variant={compact ? 'map' : 'list'} className={cn('group relative overflow-hidden bg-white transition duration-200', compact ? 'w-full rounded-none border-0 shadow-none' : 'min-h-[300px] cursor-pointer rounded-2xl border shadow-sm focus-within:ring-2 focus-within:ring-primary/40 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md', !compact && (highlighted ? 'border-2 border-red-500 shadow-[0_0_0_4px_rgba(220,38,38,0.18),0_16px_40px_rgba(22,50,79,0.32)] hover:border-red-500 hover:shadow-[0_0_0_4px_rgba(220,38,38,0.18),0_16px_40px_rgba(22,50,79,0.32)]' : 'border-border'))} onMouseEnter={() => onHover?.(building.id)} onMouseLeave={() => onHover?.(null)} onClickCapture={(event) => { const target = event.target as HTMLElement; if (target.closest('button, a')) return; onSelect?.(building.id); }}>
      <div className="grid min-h-[300px] grid-cols-1">
        {!compact && <div className="relative min-h-[260px] overflow-hidden bg-muted sm:min-h-[340px]">
          ...
          <Link href={hasMorePhotos ? `/buildings/${building.slug}` : `/rent-request?${currentOptionsContext}`} onClick={(event) => event.stopPropagation()} className="absolute bottom-4 left-4 z-20 max-w-[calc(100%-2rem)] truncate rounded-md bg-primary px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-primary-foreground shadow-sm hover:bg-primary-hover sm:text-sm">View Photos</Link>
          <button type="button" className="absolute right-4 top-4 z-20 inline-flex min-h-[64px] flex-col items-center justify-center gap-0.5 rounded-xl bg-white/95 px-4 py-2 text-xs font-semibold text-navy shadow-md backdrop-blur transition hover:bg-white" aria-label={savedAndCompared ? `Remove ${building.name} from saved and compare` : `Save ${building.name} and add to compare`} aria-pressed={savedAndCompared} onClick={(event) => { event.stopPropagation(); const next = !savedAndCompared; onFavoriteChange?.(building, next); onCompareChange?.(building, next); }}><Heart className={cn('h-6 w-6', savedAndCompared && 'fill-destructive text-destructive')} /><span>Save and Compare</span></button>
        </div>}
