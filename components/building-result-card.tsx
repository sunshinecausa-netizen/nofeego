    ...
    return (
      <article data-building-id={building.id} className={cn('group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-sm transition duration-200 focus-within:ring-2 focus-within:ring-primary/40 hover:-translate-y-0.5 hover:shadow-md lg:h-[540px]', highlighted && 'ring-2 ring-primary/40 shadow-[0_12px_30px_rgba(22,50,79,0.22)]')} onMouseEnter={() => onHover?.(building.id)} onMouseLeave={() => onHover?.(null)} onClickCapture={(event) => { const target = event.target as HTMLElement; if (target.closest('button, a, [data-copyable]')) return; onSelect?.(building.id); }}>
        <div className="relative aspect-[1.55/1] shrink-0 overflow-hidden bg-muted lg:aspect-auto lg:h-[52%]">
          ...
          <Link href={hasMorePhotos ? `/buildings/${building.slug}` : `/rent-request?${currentOptionsContext}`} onClick={(event) => event.stopPropagation()} className="absolute bottom-4 left-4 z-20 max-w-[calc(100%-2rem)] truncate rounded-md bg-primary px-3 py-2 text-xs font-bold uppercase tracking-[0.12em] text-primary-foreground shadow-sm hover:bg-primary-hover">{hasMorePhotos ? 'View More Photos' : 'Request More Photos'}</Link>
        ...
        <div className="flex min-h-0 flex-1 flex-col px-4 py-3.5">
          <div className="grid gap-1.5">
            ...
            <p className="truncate text-xs font-medium text-muted-foreground">{building.neighborhood ?? building.borough ?? 'New York metro'}</p>
    ...
    <article data-building-id={building.id} className={cn('group relative overflow-hidden bg-white transition duration-200', compact ? 'w-full rounded-none border-0 shadow-none' : 'min-h-[300px] cursor-pointer rounded-2xl border shadow-sm focus-within:ring-2 focus-within:ring-primary/40 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md', !compact && (highlighted ? 'border-2 border-primary shadow-[0_0_0_4px_rgba(239,145,0,0.2),0_16px_40px_rgba(22,50,79,0.32)] hover:border-primary hover:shadow-[0_0_0_4px_rgba(239,145,0,0.2),0_16px_40px_rgba(22,50,79,0.32)]' : 'border-border'))} onMouseEnter={() => onHover?.(building.id)} onMouseLeave={() => onHover?.(null)} onClickCapture={(event) => { const target = event.target as HTMLElement; if (target.closest('button, a, [data-copyable]')) return; onSelect?.(building.id); }}>
      <div className="grid min-h-[300px] grid-cols-1">
        {!compact && <div className="relative min-h-[260px] overflow-hidden bg-muted sm:min-h-[340px]">
          ...
          <Link href={hasMorePhotos ? `/buildings/${building.slug}` : `/rent-request?${currentOptionsContext}`} onClick={(event) => event.stopPropagation()} className="absolute bottom-4 left-4 z-20 max-w-[calc(100%-2rem)] truncate rounded-md bg-primary px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-primary-foreground shadow-sm hover:bg-primary-hover sm:text-sm">{hasMorePhotos ? 'View More Photos' : 'Request More Photos'}</Link>
        ...
        <div className="relative flex min-w-0 flex-col p-5 sm:p-6">
          <div className={cn('min-w-0', compact && (onClose ? 'pr-52' : 'pr-44'))}>
            ...
            <p className={cn('mt-1 truncate font-medium text-muted-foreground', compact ? 'text-sm' : 'text-base')}>{building.neighborhood ?? building.borough ?? 'New York metro'}</p>
          </div>
