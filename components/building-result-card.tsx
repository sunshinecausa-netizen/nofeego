        ...
        <div className="flex min-h-0 flex-1 flex-col px-4 py-3.5">
          <div className="grid gap-1.5">
            ...
            <div className="grid grid-cols-4 border-y border-border/70 py-2" aria-label="Starting base rents by apartment type">
              {BEDROOM_PRICE_LABELS.map(([bedroom, label], index) => { const minimum = inventory?.bedroomMinimums[bedroom]; const count = inventory?.bedroomAvailableCounts[bedroom] ?? 0; return <div key={bedroom} className={cn('min-w-0 px-1 text-center', index > 0 && 'border-l border-border')}><p className="truncate text-xs font-semibold text-navy">{label}</p><p className="mt-0.5 text-[8px] font-semibold uppercase tracking-wide text-muted-foreground">Starting from</p><p className="truncate text-sm font-bold text-navy">{minimum != null ? `$${formatCurrency(minimum)}` : '—'}</p><p className="mt-0.5 truncate text-[9px] font-semibold text-primary">{count} available</p></div>; })}
            </div>
      ...
      <div className="grid min-h-[300px] grid-cols-1">
        ...
        <div className="relative flex min-w-0 flex-col p-5 sm:p-6">
          ...
          <div className="mt-3 grid grid-cols-4 border-y border-border/70 py-4" aria-label="Starting base rents by apartment type">
            {BEDROOM_PRICE_LABELS.map(([bedroom, label], index) => { const minimum = inventory?.bedroomMinimums[bedroom]; const count = inventory?.bedroomAvailableCounts[bedroom] ?? 0; return <div key={bedroom} className={cn('min-w-0 px-2 text-center', index > 0 && 'border-l border-border')}><p className="whitespace-nowrap text-sm font-medium text-navy">{label}</p><p className="mt-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Starting from</p><p className={cn('whitespace-nowrap font-semibold text-navy', compact ? 'text-base' : 'text-xl')}>{minimum != null ? `$${formatCurrency(minimum)}` : '—'}</p><p className="mt-1 whitespace-nowrap text-[11px] font-semibold text-primary">{count} available</p></div>; })}
          </div>
