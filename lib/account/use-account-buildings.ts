...
export function useAccountBuildings(ids: string[]) {
  ...
  useEffect(() => { let active = true; const buildingIds = key ? key.split(',') : [];
    ...
    void (async () => { setLoading(true);
      ...
      const [{ data: availabilityRows }, { data: rentRows }, { data: unitCountRows }] = await Promise.all([supabase.from('public_building_availability').select('*').in('building_slug', slugs), supabase.from('public_building_rent_summary').select('*').in('building_slug', slugs), supabase.from('public_building_unit_counts').select('*').in('building_slug', slugs)]);
      ...
      const unitCounts = new Map((unitCountRows ?? []).map((row) => [row.building_slug, { total: row.available_unit_count, byBedroom: { 0: row.studio_available_count, 1: row.one_bed_available_count, 2: row.two_bed_available_count, 3: row.three_bed_available_count } }]));
      ...
      setBuildings(mapped); setInventoryByBuilding(Object.fromEntries(mapped.map((b) => [b.id, { availabilityStatus: bySlug.get(b.slug) ?? 'unavailable', bedroomMinimums: rents.get(b.slug) ?? {}, bedroomAvailableCounts: unitCounts.get(b.slug)?.byBedroom ?? {}, availableCount: unitCounts.get(b.slug)?.total ?? 0 }]))); setLoading(false);
    })(); return () => { active = false; };
