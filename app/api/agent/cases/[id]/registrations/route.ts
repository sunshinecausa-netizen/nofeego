...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  const webhook=process.env.PROPERTY_INVITE_DELIVERY_WEBHOOK;
  ...
  if(!webhook)return NextResponse.json({item:registration,delivery:{status:'manual_required',message:'Registration saved. Ask an Admin to deliver the invitation securely.'}},{status:202});
  ...
  if(!delivery.ok)return NextResponse.json({item:registration,delivery:{status:'manual_required',message:'Registration saved, but automatic delivery failed. Contact an Admin.'}},{status:202});return NextResponse.json({item:registration,delivery:{status:'delivered'}},{status:201});
}
