...
const inquirySchema = z.object({ idempotencyKey: z.string().uuid(), buildingId: z.union([z.string().uuid(), z.literal('')]).transform((value) => value || null), requestType: z.enum(['entire_place', 'roommate']), message: z.string().trim().max(3000).optional().default(''), moveInDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional(), monthlyBudget: z.number().positive().max(1_000_000).nullable().optional(), contactName: z.string().trim().max(120).optional().default(''), contactEmail: z.string().email().max(254), contactPhone: z.string().trim().max(40).optional().default(''), bedrooms: z.string().trim().max(30).optional().default(''), leaseTermMonths:z.number().int().min(1).max(36).nullable().optional(), contactPreference:z.enum(['email','phone','text']).nullable().optional(), roommatePreferences: z.string().trim().max(3000).optional().default(''), selectedFloorPlan: z.string().trim().max(120).optional().default(''), displayedStartingRent: z.number().positive().max(1_000_000).nullable().optional(), preferredUnitType: z.string().trim().max(60).optional().default(''), attribution:attributionSchema.nullable().optional() }).strict();
...
export async function POST(request: Request) {
  ...
  let data = existing;
  if (!data) {
    const saved = await auth.supabase.from('inquiries').insert({ user_id: auth.user.id, idempotency_key: input.idempotencyKey, building_id: input.buildingId, request_type: input.requestType, message: input.message || null, move_in_date: input.moveInDate ?? null, monthly_budget: input.monthlyBudget ?? null, contact_name: input.contactName, contact_email: input.contactEmail, contact_phone: input.contactPhone || null, bedrooms: input.bedrooms || null, lease_term_months:input.leaseTermMonths??null, contact_preference:input.contactPreference??'email', roommate_preferences: input.roommatePreferences || null, status: 'Submitted' }).select('id, status, created_at').single();
