...
const actionSchema=z.object({action:z.enum(['approve','simulate_send','simulate_failure','retry']),outboxId:z.string().uuid()}).strict();
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const {data,error}=await auth.supabase.rpc('simulate_property_contact_send',{p_outbox_id:v.outboxId,p_fail:v.action==='simulate_failure'});
  if(error)return accountError('OUTBOX_SIMULATION_REJECTED','Approve the draft before simulating delivery.',403);
  return NextResponse.json({item:data,externalDelivery:false});
}
