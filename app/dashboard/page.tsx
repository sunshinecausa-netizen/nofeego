...
import { GitCompareArrows, Heart, Loader2, MessageSquare, Users } from 'lucide-react';
import { useAuth } from '@/lib/auth-context';
import { useTenantData } from '@/lib/account/tenant-data-context';
import { accountFetch } from '@/lib/account/client';

export default function DashboardPage() {
  const router = useRouter(); const { user, profile, loading: authLoading } = useAuth(); const { favoriteIds, compareIds } = useTenantData();
  const [requests, setRequests] = useState<Array<{ id: string; request_type: string | null; status: string; created_at: string }>>([]);
  const [roommateInterestCount, setRoommateInterestCount] = useState(0);
  useEffect(() => { if (!authLoading && !user) router.replace('/sign-in?next=/dashboard'); }, [authLoading, router, user]);
  useEffect(() => {
    if (authLoading || !user || !profile || profile.account_role === 'tenant') return;
    const roleHome = profile.account_role === 'agent' ? '/agent/cases' : profile.account_role === 'property' ? '/property/registrations' : '/admin/rental-cases';
